...
@section('pagetitle')
    <title>{{ config('app.name', 'Laravel') }} | Kantor Cabang</title>
@endsection
        ...
        buttons: [
            {
              ...
              action: function ( e, dt, node, config ) {
                  window.location = '{{ url('branch/create') }}'
              }
            },  
            { extend: 'colvis', text: 'Column' },'copy', 'csv', 'excel', 'pdf', 'print',
            {
              ...
              action: function ( e, dt, node, config ) {
                  window.location = '{{ url('branch/csvall') }}'
              }
            },
            {
    ...
    $('.buttons-deletemulti').click(function(){
      ...
      if(deleteids_arr.length > 0){
        ...
        if (confirmdelete == true) {
          window.location = '{{ url('branch/destroymulti?id=') }}'+deleteids_str
        } 
      }
      });
});
